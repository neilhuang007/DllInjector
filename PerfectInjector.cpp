...
// Function to display the list of processes and get user selection
std::string GetUserSelectedProcess()
{
    std::vector<std::string> processes = ListRunningProcesses();
    std::sort(processes.begin(), processes.end());

    std::cout << "Select the target process by number:\n";
    ...
    while (true)
    {
        std::cout << "Enter the number of the target process: ";
        std::cin >> selection;
...
int main(int argc, char** argv)
{
    ...
    if (!ProcessName.size())
    {
        ProcessName = GetUserSelectedProcess();
    }
    if (!DllPath.size())
    {
        std::cout << "Enter the path to the module: ";
        std::cin >> std::ws;
